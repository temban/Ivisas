<template>
  ...
  <div class="slide-wrapper" v-for="(slide, index) in slides" :key="slide.id" v-bind:class="{ active: index === currentSlide }" 
  v-bind:style="{ 'z-index': (slides.length - index), 'background-image': 'url(' + (`${slide.bgImg}`)+')' }"
    ...
    <div class="slide-inner">
      ...
      <div class="slide-content">
        <h5 class="slide-content-text">
          <p >{{ slide.headlineFirstLine }}</p>
          <p >{{ slide.headlineSecondLine }}</p>
        </h5>
        <!-- <button id="cancel-button" class="popup-button-cta" type="button">Special Cameroun</button> -->
    </div>
      <h2 class="slide-side-text"><span>{{ slide.sublineFirstLine }} / </span><span>{{ slide.sublineSecondLine }}</span></h2>
    </div>
  </div>
  <div class="controls-container">
    <button class="controls-button" v-for="(slide, index) in buttons" :key="slide.id" v-bind:class="{ active: index === currentSlide }" v-on:click="updateSlide(index)"
            v-on:mouseover="stopRotation"
      v-on:mouseout="startRotation">{{ slide.but }}</button>
  </div>
  <div class="pagination-container">
    <span class="pagination-item" v-for="(slide, index) in slides" :key="slide.id" v-bind:class="{ active: index === currentSlide }" v-on:click="updateSlide(index)"
          v-on:mouseover="stopRotation"
  ...
  export default {
    ...
    buttons:[
        ...
        {
         but:"Special Cameroun",
        },
    ],
    slides: [
      {
        headlineFirstLine: "Concentrez vous",
        headlineSecondLine: "sur votre bonheur.",
        sublineFirstLine: "IVISAS-AFFAIRES",
        sublineSecondLine: "TOJOURS PLUS PRET DE VOUS.",
        bgImg: require('@/assets/images/visa-requirements-retouch.jpg') ,
        rectImg: "https://i.postimg.cc/vTW0XkvM/slide-rect0.jpg"
      },
      {
        headlineFirstLine: "Votre satisfaction ",
        headlineSecondLine: "est notre priorité",
        sublineFirstLine: "IVISAS-AFFAIRES",
        sublineSecondLine: "TOJOURS PLUS PRET DE VOUS.",
        bgImg: require("@/assets/air-transport-1.jpg"),
        rectImg: "https://i.postimg.cc/ryWZ8R2b/slide-rect1.jpg"
      },
      {
        headlineFirstLine: "Cameroun:",
        headlineSecondLine: "l'Afrique en miniature.",
        sublineFirstLine: "IVISAS-AFFAIRES", 
        sublineSecondLine: "TOJOURS PLUS PRET DE VOUS.",
        bgImg: require("@/assets/img/Yaounde-Cameroon.jpg"),
        rectImg: require("@/assets/img/flag1.jpg"),
      }
    ]
      };
    },
...
.slide {
    &-wrapper {
        ...
        &:nth-child(1) {
            background-color: rgba(115, 129, 153, 0.367);
            &:before {
                background-color: rgba(115, 129, 153, 0.338);
            }
            .slide-content-text {
        ...
        &:nth-child(2) {
            background-color: rgba(144, 171, 184, 0.405);
            &:before {
                background-color: rgba(144, 171, 184, 0.172);
            }
            .slide-content-text {
        ...
        &:nth-child(3) {
            background-color: rgba(86, 125, 156, 0.087);
            &:before {
                background-color: rgba(86, 125, 156, 0.084);
            }
            .slide-content-text {
    ...
    &-content {
        ...
        &-cta {
            ...
            margin-top: 4.5rem;
            margin-left: calc((.3) * 70vh + 4.5rem);
            padding: .9rem 1.2rem;
...
#cancel-button {
    ...
    width: 200px;
    border-radius: 4px;
    background-color: rgba(45, 44, 44, 0.778);
    outline: none;
    margin: 35px;
    transition: 0.2s;
    cursor: pointer;
...
@media (max-width: 630px) {
  .slide-content .slide-content-text p{
 font-size: 1.6rem ;
  }
}
